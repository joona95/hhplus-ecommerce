...
| 지표 | 값 | 목표 | 상태·해석 |
| --- | --- | --- | --- |
| **실패율** | **0 %** (0 / 377 070) | < 5 % | ✅ 정상 |
| **p95 지연** | **8.45 s** | < 0.5 s | ❌ 17 × 초과 |
| p90 / 중앙값 | 7.68 s / 3.67 s | — | 대기 구간 길다 |
| 평균 | 3.82 s | — | 전체 지연 높음 |
| 최대 | 11.2 s | — | 커넥션/락 타임아웃 근접 |
| **처리량** | 966 RPS | 평시 목표 1.8 k | 54 % 수준 |
| VU 효율 | 6 500 VU → 966 RPS (0.15 RPS/VU) | ≥ 1 RPS/VU | **대기 지옥** |

선착순 쿠폰은 3번의 피크 RPS가 30초 안에 몰린다고 생각했을 때 선착순 쿠폰 발급이 무사히 잘 작동하는지 확인하고, 선착순 쿠폰 발행 시와 같은 이벤트 상황에 어느 정도의 인프라를 미리 구축하여 대응할 수 있을지 살펴보고자 하였습니다.

피크 타입을 견딜 정도의 인프라가 현재 아니기 때문에 이를 위해서 지금 수준 이상의 인프라 구축이 필요할 것임을 알 수 있습니다.
